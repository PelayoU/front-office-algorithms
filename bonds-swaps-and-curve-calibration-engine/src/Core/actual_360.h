...
public:
     ...
     * @return Número de días entre las dos fechas; se usa la función days() de boost::gregorian::date. Devuelve un short, que es un entero de 16 bits para optimizar el espacio en memoria.
     */
     ...
     * para convertir el string en un objeto boost::gregorian::date, y luego calcula el número de días entre esas dos fechas usando sobrecarga de funciones, es decir, llamando a la funcion compute_daycount() que calcula el numero de días entre dos fechas de tipo boost::gregorian::date.
